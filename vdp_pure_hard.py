...
# 5. Diagnostics
# =========================================================
def check_hard_constraints(model, name, x0, v0):
    t0 = torch.tensor([[0.0]], dtype=torch.float32, requires_grad=True)
    x = hard_output(model, t0, x0, v0, t0=0.0)
    ...
    print(
        f"{name} hard IC check: "
        f"x(0)={x.item():.8f}, target={x0:.8f} | "
        f"x'(0)={x_t.item():.8f}, target={v0:.8f}"
    )
...
if __name__ == "__main__":
    ...
    check_hard_constraints(hard_result["model"], "Hard PINN", x0, v0)
    ...
    plt.grid(False)
    plt.tight_layout()
    plt.savefig("vdp_pure_hard_solution.png", dpi=300, bbox_inches="tight")
    plt.show()
    ...
    plt.grid(False)
    plt.tight_layout()
    plt.savefig("vdp_pure_hard_abs_l2.png", dpi=300, bbox_inches="tight")
    plt.show()
    ...
    plt.grid(False)
    plt.tight_layout()
    plt.savefig("vdp_pure_hard_rel_l2.png", dpi=300, bbox_inches="tight")
    plt.show()
