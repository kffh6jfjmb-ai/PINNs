...
class RFFLayer(nn.Module):
    def __init__(self, out_features=64, sigma=1.0):
        super().__init__()
        if out_features % 2 != 0:
            raise ValueError("out_features must be even, since sin and cos are paired.")

        # Keeping the same seed across sigma values means that only the scale changes.
        B = torch.randn(1, out_features // 2) * sigma
...
def hard_output(model, t):
    """
    Hard ansatz for x(0)=0 and x'(1)=1.
    """
    x_hat = model(t)
...
def run_harmonic_sigma_study():
    ...
    for n in n_values:
        ...
        for sigma in sigma_values:
            ...
            # Soft RFF-PINN
            set_seed(SEED)
            soft_rff = SoftRFFPINN(rff_features=rff_features, sigma=sigma, width=width)
            soft_result = train_soft_model(
                model=soft_rff,
                model_name=f"Soft RFF n={n}, sigma={sigma}",
                n=n,
                t_min=t_min,
                t_max=t_max,
                t_ref=t_ref,
            ...
            histories[n]["Soft RFF-PINN"][sigma] = soft_result
            rows.append({
            ...
            # Hard RFF-PINN
            set_seed(SEED)
            hard_rff = HardRFFPINN(rff_features=rff_features, sigma=sigma, width=width)
            hard_result = train_hard_model(
                model=hard_rff,
                model_name=f"Hard RFF n={n}, sigma={sigma}",
                n=n,
                t_min=t_min,
                t_max=t_max,
                t_ref=t_ref,
            ...
            histories[n]["Hard RFF-PINN"][sigma] = hard_result
            rows.append({
                "problem": "Harmonic oscillator",
                "n": n,
                "model": "Hard RFF-PINN",
    ...
    plot_final_errors(rows, n_values, sigma_values)
    plot_histories(histories)


def plot_final_errors(rows, n_values, sigma_values):
    fig, axes = plt.subplots(1, len(n_values), figsize=(12, 4.5), sharey=True)

    if len(n_values) == 1:
        axes = [axes]

    sigma_labels = ["0.25", "0.5", "1", "2", "4", "8"]
    ...
    for ax, n in zip(axes, n_values):
        ...
        # Show every tested sigma value explicitly on the x-axis.
        ax.set_xticks(sigma_values)
        ax.set_xticklabels(sigma_labels)
        ax.set_xlabel(r"Fourier feature scale $\sigma$")
        ax.set_title(rf"$n={n}$")
...
def plot_histories(histories):
    for n, n_histories in histories.items():
        for model_name, model_histories in n_histories.items():
            ...
            for sigma, result in model_histories.items():
                plt.semilogy(
                    result["l2_epochs"],
                    result["rel_l2"],
                    lw=2,
