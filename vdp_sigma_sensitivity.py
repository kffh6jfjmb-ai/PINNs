...
def run_vdp_sigma_study():
    ...
    for sigma in sigma_values:
        ...
        soft_result = train_soft_model(
            model=soft_rff,
            model_name=f"Soft RFF sigma={sigma}",
            mu=mu,
            t_min=t_min,
            t_max=t_max,
            x0=x0,
        ...
        histories["Soft RFF-PINN"][sigma] = soft_result
        rows.append({
            "problem": "Van der Pol",
            "model": "Soft RFF-PINN",
            "sigma": sigma,
        ...
        hard_result = train_hard_model(
            model=hard_rff,
            model_name=f"Hard RFF sigma={sigma}",
            mu=mu,
            t_min=t_min,
            t_max=t_max,
            x0=x0,
        ...
        histories["Hard RFF-PINN"][sigma] = hard_result
        rows.append({
            "problem": "Van der Pol",
            "model": "Hard RFF-PINN",
            "sigma": sigma,
    ...
    plot_final_errors(rows, sigma_values)
    plot_histories(histories)


def plot_final_errors(rows, sigma_values):
    soft = [r["final_rel_l2"] for r in rows if r["model"] == "Soft RFF-PINN"]
    hard = [r["final_rel_l2"] for r in rows if r["model"] == "Hard RFF-PINN"]

    plt.figure(figsize=(7, 4.5))
    plt.loglog(sigma_values, soft, marker="o", lw=2, label="Soft RFF-PINN")
    plt.loglog(sigma_values, hard, marker="s", lw=2, label="Hard RFF-PINN")
    plt.xticks(sigma_values, ["0.25", "0.5", "1", "2", "4", "8"])
    plt.xlabel(r"Fourier feature scale $\sigma$")
